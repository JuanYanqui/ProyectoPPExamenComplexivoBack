...
public interface ITutorEmpresarialDao extends JpaRepository<Tutor_Empresarial,Long> {
    ...
    @Query("SELECT t.idTutorEmpresarial ,e.nombreEmpresa, CONCAT(u.nombres, ' ', u.apellidos) AS nombre, u.correo, t.numeroContacto FROM Tutor_Empresarial t JOIN Empresa e ON e.idEmpresa = t.empresa.idEmpresa JOIN Usuario u ON u.idUsuario = t.usuario_empresarial.idUsuario WHERE t.estado=TRUE ORDER BY t.idTutorEmpresarial ASC")
